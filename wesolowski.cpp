...
void generate_prime(mpz_class& rop, gmp_randstate_t& rstate, const mp_bitcnt_t& n){
        mpz_urandomb(rop, rstate, n);
        mpz_nextprime(rop, rop);
}

void Wesolowski::hash_prime(mpz_class l, const mpz_class input)
{
...
void Wesolowski::generate(mpz_class& dest) {
        mpz_urandomm(dest, rstate, N);
}

Proof Wesolowski::evaluate(mpz_class l, mpz_class pi, const mpz_class x,
                           const long challenge) {

        // HERE WE START THE EVALUATION
        auto start_eval = std::chrono::high_resolution_clock::now();

        mpz_class exp_challenge;
...
bool Wesolowski::naive_verify(mpz_class x, long challenge, mpz_class l, mpz_class pi) {
...
void exponentiation(mpz_class ret, mpz_class radix, mpz_class exp, mpz_class N)
{
        mpz_powm(ret, radix, exp, N);
}

void exponentiation_euclidian(mpz_class ret, mpz_class radix, mpz_class exp, mpz_class N, int k)
{
        mpz_class A, B;
        mpz_class and_mod;
...
bool Wesolowski::parallel_verify(mpz_class x, long challenge, mpz_class l, mpz_class pi) {
...
bool Wesolowski::parallel_diff_verify(mpz_class x, long challenge, mpz_class l, mpz_class pi) {
...
void get_bit(mpz_class ret, mpz_class e, int j)
{
        mpz_fdiv_q_2exp(ret, e, j);
        mpz_mod_ui(ret, ret, 2);
}

bool test_while(mpz_class x, mpz_class y, int j)
{
        if(mpz_classstbit(x, j) == 0 && mpz_classstbit(y, j) == 0)
        {
                return true;
        }
        else
...
int filter(mpz_class e, int j, int J)
{
        mpz_class ret;
                mpz_fdiv_q_2exp(ret, e, J);
...
mpz_class multi_exponentiation(mpz_class x, mpz_class y, mpz_class a, mpz_class b, mpz_class N, int k, int w)
{
        int pow_w = pow(2, w);
        ...
        while(j>=0)
        {
                if(mpz_classstbit(a, j) == 0 && mpz_classstbit(b, j) == 0)
                {
                        R = (R*R)%N;
                        j--;
                }
...
bool Wesolowski::optimized_verify(mpz_class x, long challenge, mpz_class l, mpz_class pi, int w) {
        ...
        if(mpz_cmp(R.get_mpz_class(), y_saved) == 0) {
                auto finish_verif = std::chrono::high_resolution_clock::now();
